...
function AddTaskForm(workerId) {
    window.location.href = `/tasks/add/${workerId}`; // Redirect to add task page with workerId
}
...
function viewTasks(workerId) {
    fetch(`/tasks/${workerId}/view`)
        ...
        .then(tasks => {
            ...
            if (tasks.length === 0) {
                taskList.innerHTML = '<tr><td colspan="4">No tasks assigned</td></tr>'; // Handle empty tasks
                return;
            }
            ...
            tasks.forEach(task => {
                taskList.innerHTML += `
                    <tr>
                        ...
                        <td>${new Date(task.taskDate).toLocaleDateString()}</td> <!-- Format date -->
                        <td>${task.priority}</td>
                    </tr>
                `;
            });
